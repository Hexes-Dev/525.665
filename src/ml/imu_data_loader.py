...
class IMUDataset(Dataset):
    ...
    def _normalize(self, value, sensor_name, key):
        ...
        else: # For temp (scalar)
            # The generate_norm_params script saves tmp as {'mean': x, 'std': y} in global/per-sensor
            # Let's check the structure again. Actually calculate_stats returns a dict for all.
            # Wait, looking at my previous write: 
            # return { "acc": {...}, ..., "tmp": {"mean": float, "std": float} }
            # So stats is always a dict if we follow that logic.
            # But in the generate_norm_params script I might have simplified it.
            # Let's assume the structure from calculate_stats: { 'mean': ..., 'std': ... }
    ...
    def __getitem__(self, idx: int) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        for r in imu_readings:
            ...
            # Find closest GPS reading for current IMU timestamp
...
def split_imu_datasets(db_path: str, train_split: float = 0.8, seed: int = None, **kwargs) -> tuple[IMUDataset, IMUDataset]:
    ...
    """
    # Temporary dataset to identify all possible anchors
    ...
    train_ds = IMUDataset(db_path, anchors=train_anchors, **kwargs)
    test_ds = IMUDataset(db_path, anchors=test_anchors, **kwargs)
